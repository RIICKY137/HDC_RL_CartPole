...
from dqn_cartpole_v2.utils import make_env, resolve_device, set_global_seed, write_json


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Evaluate a trained HDC-DQN checkpoint on CartPole-v1.")
...
def load_agent(checkpoint_path: Path, device: torch.device) -> tuple[HDCDQNAgent, HDCDQNConfig]:
    ...
    config = HDCDQNConfig(
        ...
        hdc_dim=checkpoint_config["hdc_dim"],
        hdc_n_bins=checkpoint_config["hdc_n_bins"],
        hdc_bounds=tuple(tuple(bound) for bound in checkpoint_config["hdc_bounds"]),
