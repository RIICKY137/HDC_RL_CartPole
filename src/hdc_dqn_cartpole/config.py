...
class HDCDQNConfig:
    ...
    hdc_n_bins: int = 10
    hdc_bounds: tuple[tuple[float, float], ...] = (
        (-2.4, 2.4),
        (-3.0, 3.0),
        (-0.418, 0.418),
        (-4.0, 4.0),
    ...
    checkpoint_path: Path = field(
        default_factory=lambda: Path("artifacts/checkpoints/cartpole_hdc_dqn.pt")
    )
    metrics_path: Path = field(default_factory=lambda: Path("artifacts/hdc_train_metrics.json"))
    device: str = "auto"
    log_every: int = 10
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        payload["hdc_bounds"] = [list(bound) for bound in self.hdc_bounds]
        return payload
