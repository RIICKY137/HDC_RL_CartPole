...
from .agent import HDCDQNAgent
from .config import HDCDQNConfig
from .evaluate import evaluate_policy
from dqn_cartpole_v2.utils import ensure_parent_dir, make_env, resolve_device, rolling_average, set_global_seed, write_json


def build_parser() -> argparse.ArgumentParser:
    defaults = HDCDQNConfig()
    parser = argparse.ArgumentParser(description="Train an HDC-encoded DQN agent on CartPole-v1.")
    ...
    parser.add_argument("--hdc-dim", type=int, default=defaults.hdc_dim)
    parser.add_argument("--hdc-n-bins", type=int, default=defaults.hdc_n_bins)
...
def config_from_args(args: argparse.Namespace) -> HDCDQNConfig:
    config = HDCDQNConfig()
    return replace(
        config,
        seed=args.seed,
...
def train(config: HDCDQNConfig) -> dict[str, Any]:
    set_global_seed(config.seed)
    device = resolve_device(config.device)
    env = make_env(config.environment_id, config.seed)
    state_size = int(env.observation_space.shape[0])
    action_size = int(env.action_space.n)
    agent = HDCDQNAgent(state_size=state_size, action_size=action_size, config=config, device=device)
    ...
    for episode in range(1, config.episodes + 1):
        ...
        should_validate = episode % config.validation_interval == 0 or episode == config.episodes
        if should_validate:
            ...
            if validation_metrics["mean_reward"] >= best_validation_reward:
                ...
                save_checkpoint(config.checkpoint_path, agent, config, checkpoint_summary)
